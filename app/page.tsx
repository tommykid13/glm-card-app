...
+
+      if (data?.poster) { setLayout('poster'); setPoster(data.poster); return; }
+      if (Array.isArray(data?.cards)) { setLayout('list'); setCards(data.cards); return; }
...
+    }
+    if (layout === 'list') {
+      const t = cards.map((c: any, i: number) =>
+        `#${i + 1} ${c.icon || ''} ${c.title}\n${c.description}\n標籤：${(c.tags || []).join('/') || '-'}`,
+      ).join('\n\n');
+      navigator.clipboard.writeText(t);
+    }
...
+              <input type="number" min={1} max={20} className="w-full rounded border p-2"
...
+            <button className={`rounded border px-3 py-1 text-sm ${layout==='poster'?'border-amber-400 bg-amber-100':'border-gray-200 hover:bg-gray-50'}`} onClick={()=>setLayout('poster')}>海報模式</button>
+            <button className={`rounded border px-3 py-1 text-sm ${layout==='list'?'border-amber-400 bg-amber-100':'border-gray-200 hover:bg-gray-50'}`} onClick={()=>setLayout('list')}>列表模式</button>
+          </div>
...
+            <button className="rounded border px-3 py-1 text-sm hover:bg-gray-50" onClick={generate} disabled={loading}>重新生成</button>
+            <button className="rounded border px-3 py-1 text-sm hover:bg-gray-50" onClick={copyJSON}>複製JSON</button>
+            <button className="rounded border px-3 py-1 text-sm hover:bg-gray-50" onClick={copyAllText}>複製全部文案</button>
+            {layout === 'poster' && poster ? (
+              <>
+                <button className="rounded border px-3 py-1 text-sm hover:bg-gray-50" onClick={exportPNG}>導出PNG</button>
+                <button className="rounded border px-3 py-1 text-sm hover:bg-gray-50" onClick={exportHTML}>下載HTML</button>
+              </>
+            ) : null}
+          </div>
+
...
+                </h2>
+                {poster.subtitle ? <p className="text-sm text-gray-600">{poster.subtitle}</p> : null}
+                <div className="space-y-3">
+                  {(poster.sections || []).map((s: any, i: number) => (
+                    <section key={i}>
+                      <h3 className="font-semibold">{s.icon} {s.heading}</h3>
...
+                        {(poster.compare.left.bullets || []).map((b: string, i: number) => <p key={i}>• {b}</p>)}
+                      </div>
+                      <div>
+                        <h4 className="underline">{poster.compare.right.title}</h4>
+                        {(poster.compare.right.bullets || []).map((b: string, i: number) => <p key={i}>• {b}</p>)}
+                      </div>
+                    </div>
+                  </section>
+                ) : null}
...
+                      <div key={i}><h4>{g.icon} {g.title}</h4><p>{g.text}</p></div>
+                    ))}
+                  </section>
+                ) : null}
+                {poster.takeaway ? (
...
+                    {poster.takeaway.question ? <p>{poster.takeaway.question}</p> : null}
+                  </section>
+                ) : null}
+                <details className="mt-2">
+                  <summary className="cursor-pointer text-xs text-gray-400">調試：查看 poster JSON</summary>
+                  <pre className="max-h-64 overflow-auto rounded bg-gray-50 p-2 text-xs">{JSON.stringify(poster, null, 2)}</pre>
+                </details>
+              </div>
+            ) : (
...
+                  <div className="font-semibold">{c.icon || ''} {c.title}</div>
+                  <p className="text-sm">{c.description}</p>
+                  <div className="mt-1 flex flex-wrap gap-2 text-xs text-gray-600">
+                    {(c.tags || []).map((t: string, j: number) => (
+                      <span key={j} className="rounded-full border px-2 py-0.5">{t}</span>
+                    ))}
+                  </div>
+                </div>
+              ))}
...
+          )}
+
+          {err ? <p className="mt-3 text-xs text-red-500">錯誤：{String(err)}</p> : null}
+        </section>
+      </section>
+    </main>
+  );
